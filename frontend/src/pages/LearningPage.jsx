...
function LearningPage() {
  ...
  const savedProgress = subject
    ? savedList.find((item) => item.subject === subject && item.difficulty === difficulty) || null
    : null
  ...
  useEffect(() => {
    ...
    const loadLearningState = async () => {
      setLoading(true)
      try {
        const [{ data: overviewData }, { data: progressData }] = await Promise.all([
        ...
        const progresses = Array.isArray(progressData?.data) ? progressData.data : []
        const nextDailyUsed = overview.dailyUsed || 0
        const nextRemaining = isFree ? Math.max(0, FREE_DAILY_LIMIT - nextDailyUsed) : Infinity
        const nextOptions = COUNT_OPTIONS.filter((value) => value <= nextRemaining)

        setDailyUsed(nextDailyUsed)
        setWrongNoteCount(overview.wrongNotesCount || 0)
        setDifficulty(overview.placementDifficulty || 'MEDIUM')
        setPlacementDone(Boolean(overview.placementDone))
        setSavedList(progresses)
        setCount(isFree && nextRemaining < 10 ? (nextOptions[0] || 0) : 10)
      } catch {
        if (!cancelled) {
          setSavedList([])
          setDailyUsed(0)
  ...
  const handleDifficultyChange = async (nextDifficulty) => {
    setDifficulty(nextDifficulty)
    try {
      await learningApi.saveLearningPreferences({
        placementDifficulty: nextDifficulty,
        placementDone,
      })
    } catch {
      // keep current UI state even if sync fails
    }
  }
  ...
  const handleDiscardAndStart = async () => {
    if (savedProgress) {
      try {
        await learningApi.deleteLearningProgress(subject, difficulty)
        setSavedList((current) => current.filter((item) => !(item.subject === subject && item.difficulty === difficulty)))
      } catch {
        alert('저장된 진행상황 삭제에 실패했습니다.')
        return
      }
    }
    handleStart()
  ...
  const handleDeleteFromList = async (progress, event) => {
    event.stopPropagation()
    try {
      await learningApi.deleteLearningProgress(progress.subject, progress.difficulty)
      setSavedList((current) => current.filter((item) => item.id !== progress.id))
    } catch {
      alert('저장된 진행상황 삭제에 실패했습니다.')
    }
  }
  ...
  if (loading) {
    return (
      <div style={{ textAlign: 'center', padding: '100px 0', color: 'var(--text-muted)' }}>
        ...
        <p style={{ fontSize: 15 }}>학습 정보를 불러오는 중...</p>
      </div>
    )
  }
    ...
    <div style={{ width: '100%' }}>
        ...
        {wrongNoteCount > 0 && (
          ...
          >
            ...
            <div style={{ textAlign: 'left' }}>
              ...
              <p style={{ fontSize: 14, fontWeight: 700, color: '#ef4444', margin: 0 }}>{wrongNoteCount}개</p>
            </div>
          </button>
        )}
      </div>
      ...
      {savedList.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {savedList.map((progress) => {
              const subjectInfo = SUBJECTS.find((item) => item.id === progress.subject)
              const progressMeta = DIFFICULTY_META[progress.difficulty] || DIFFICULTY_META.MEDIUM
              const answered = Object.keys(progress.userAnswers || {}).length
              const pct = Math.round((answered / progress.count) * 100)
              const dateStr = progress.savedAt
                ? new Date(progress.savedAt).toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })
                : ''

              return (
                <div
                  key={progress.id || `${progress.subject}_${progress.difficulty}`}
                  onClick={() => handleResumeFromList(progress)}
                  style={{
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      <span style={{ fontWeight: 700, fontSize: 14, color: 'var(--text)' }}>
                        {subjectInfo?.label || progress.subject}
                      </span>
                      <span style={{
                        background: progressMeta.bg, color: progressMeta.color,
                        borderRadius: 99, padding: '2px 9px', fontSize: 11, fontWeight: 700,
                        border: `1px solid ${progressMeta.border}`,
                      }}>
                        {progressMeta.emoji} {progressMeta.label}
                      </span>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                    ...
                    <button
                      onClick={(event) => handleDeleteFromList(progress, event)}
                      style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 20, alignItems: 'start' }}>
            ...
            {SUBJECTS.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontWeight: 800, fontSize: 15, color: active ? 'var(--primary)' : 'var(--text)', marginBottom: 5 }}>{item.label}</div>
        ...
        <div style={{ width: 280, display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                  {COUNT_OPTIONS.map((value) => {
                    const disabled = isFree && value > remaining
                    return (
                      <button
                        key={value}
                        ...
                        style={{
                          ...
                          border: `2px solid ${count === value ? 'var(--primary)' : 'var(--border)'}`,
                          background: disabled ? 'var(--bg)' : count === value ? 'var(--primary)' : 'var(--surface)',
                          color: disabled ? 'var(--text-muted)' : count === value ? '#fff' : 'var(--text-secondary)',
                          transition: 'all 0.15s',
                          boxShadow: count === value ? '0 4px 14px rgba(124,106,240,0.35)' : 'none',
                          transform: count === value ? 'scale(1.06)' : 'none',
          ...
          <button
            onClick={() => {
              if (savedProgress) setShowDiscardConfirm(true)
              else handleStart()
            }}
